...
`ozz::animation::TrackTriggeringJob` detects when track curve crosses a threshold value, aka an edge, triggering dated events. This is useful to implement time based events/notifications or states in sync with an animation. To do so, `ozz::animation::TrackTriggeringJob` considers what happens between 2 samples, not only the sampled point.
