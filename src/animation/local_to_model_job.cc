...
bool LocalToModelJob::Validate() const {
  ...
  valid &= input_end - input_begin >= num_soa_joints;
  valid &= output_end - output_begin >= num_joints;
...
bool LocalToModelJob::Run() const {
  ...
  // Input.
  const Skeleton::JointProperties* properties = skeleton->joint_properties();

  // Output.
  Float4x4* const model_matrices = output_begin;
  ...
  for (int joint = 0; joint < num_joints;) {
    ...
    const SoaTransform& transform = input_begin[joint / 4];
