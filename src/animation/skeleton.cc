...
void Save(OArchive& _archive,
  ...
  for (std::size_t i = 0; i < _count; i++) {
    int16 parent = _properties[i].parent;
    _archive << parent;
...
const math::SoaTransform* Skeleton::bind_pose_end() const {
  return bind_pose_ + ((num_joints_ + 3) / 4);
}
...
void Skeleton::Save(ozz::io::OArchive& _archive) const {
  ...
  // Stores bind poses.
  const int num_soa_joints = (num_joints_ + 3) / 4;
...
void Skeleton::Load(ozz::io::IArchive& _archive, ozz::uint32 /*_version*/) {
  ...
  // Reads bind pose.
  const std::size_t num_soa_joints = (num_joints_ + 3) / 4;
  bind_pose_ =
