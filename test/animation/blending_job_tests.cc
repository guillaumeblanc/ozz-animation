...
TEST(JobValidity, BlendingJob) {
  ...
  layers[0].begin = input_transforms;
  layers[0].end = input_transforms + 3;
  layers[1].begin = input_transforms;
  layers[1].end = input_transforms + 2;
  ...
  {  // Invalid NULL output.
    BlendingJob job;
    job.layers_begin = layers;
    job.layers_end = layers + 2;
    job.bind_pose_begin = bind_poses;
    job.bind_pose_end = bind_poses + 2;
    EXPECT_FALSE(job.Validate());
    EXPECT_FALSE(job.Run());
  }
  {  // Invalid NULL input.
    ...
    job.bind_pose_begin = bind_poses;
    job.bind_pose_end = bind_poses + 2;
    job.output_begin = output_transforms;
    job.output_end = output_transforms + 2;
    EXPECT_FALSE(job.Validate());
    EXPECT_FALSE(job.Run());
  }
  ...
  {  // Invalid input range.
    BlendingJob job;
    job.layers_begin = layers + 1;
    job.layers_end = layers;
    job.bind_pose_begin = bind_poses;
    job.bind_pose_end = bind_poses + 2;
    job.output_begin = output_transforms;
    job.output_end = output_transforms + 2;
  ...
  {  // Invalid output range.
    ...
    job.bind_pose_begin = bind_poses;
    job.bind_pose_end = bind_poses + 2;
    job.output_begin = output_transforms + 1;
    job.output_end = output_transforms;
    EXPECT_FALSE(job.Validate());
    EXPECT_FALSE(job.Run());
  }
  {  // Invalid output range, smaller output.
    ...
    job.bind_pose_end = bind_poses + 2;
    job.output_begin = output_transforms;
    job.output_end = output_transforms + 1;
    EXPECT_FALSE(job.Validate());
    EXPECT_FALSE(job.Run());
  }
  {  // Invalid output range, smaller output.
  ...
  {  // Invalid smaller input.
    ...
    job.layers_end = layers + 2;
    job.bind_pose_begin = bind_poses;
    job.bind_pose_end = bind_poses + 3;
    job.output_begin = output_transforms;
    job.output_end = output_transforms + 3;
    EXPECT_FALSE(job.Validate());
    EXPECT_FALSE(job.Run());
  }
...
TEST(Weight, BlendingJob) {
  ...
  { // All weights to 0. Expect bind pose as output.
    BlendingJob::Layer layers[2];
    layers[0].begin = input_transforms[0];
    layers[0].end = input_transforms[0] + 2;
    layers[1].begin = input_transforms[1];
    layers[1].end = input_transforms[1] + 2;
...
TEST(Normalize, BlendingJob) {
  ...
  { // Un-normalized weights < 1.f.
    ...
    layers[0].begin = input_transforms[0];
    layers[0].end = input_transforms[0] + 1;
    layers[1].weight = 0.3f;
    layers[1].begin = input_transforms[1];
    layers[1].end = input_transforms[1] + 1;

    BlendingJob job;
    job.layers_begin = layers;
    job.layers_end = layers + 2;
    job.bind_pose_begin = bind_poses;
    job.bind_pose_end = bind_poses + 1;
    ozz::math::SoaTransform output_transforms[3];
    job.output_begin = output_transforms;
    job.output_end = output_transforms + 1;
