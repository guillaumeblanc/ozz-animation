...
TEST(JobValidity, LocalToModel) {
  ...
  // NULL output
  {
    ...
    job.input_begin = input;
    job.input_end = input + 1;
    EXPECT_FALSE(job.Validate());
    EXPECT_FALSE(job.Run());
  }
  // NULL input
  {
    LocalToModelJob job;
    job.skeleton = skeleton;
    job.output_begin = output;
    job.output_end = output + 2;
    EXPECT_FALSE(job.Validate());
    EXPECT_FALSE(job.Run());
  }
  // Null skeleton.
  ...
  // Invalid output range: end < begin.
  {
    ...
    job.input_begin = input;
    job.input_end = input + 1;
    job.output_begin = output + 1;
    job.output_end = output;
    EXPECT_FALSE(job.Validate());
    EXPECT_FALSE(job.Run());
  }
  ...
  // Valid job. Bigger input & output
  {
    ...
    job.skeleton = skeleton;
    job.input_begin = input;
    job.input_end = input + 2;
    job.output_begin = output;
    job.output_end = output + 5;
    EXPECT_TRUE(job.Validate());
    EXPECT_TRUE(job.Run());
  }
