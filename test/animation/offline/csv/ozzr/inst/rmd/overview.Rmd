...
```{r xy animation skin}
ozzr::plot_animations(models, models$skinning_error, "Estimated skinning error (mm)", !params$fast)
```

```{r xy animation t}
ozzr::plot_animations(models, models$t.e, "Model space translation error (mm)", !params$fast)
```

```{r xy animation r}
ozzr::plot_animations(models, models$r.e, "Estimated rotation error (mm)", !params$fast)
```
...
bp <- compression %>%
  ggplot2::ggplot(ggplot2::aes(x=experience, y=size / 1024, color=experience, fill=experience)) +
    ...
    ggplot2::geom_text(ggplot2::aes(label = formatC(size / 1024, digits = 1, format = "f"), vjust=-0.25, show.legend = FALSE)) +
...
```{r performance, fig.asp = ozzr::plot_aspect(3)}
bp <- performance %>%
...
```{r performance_boxes, fig.asp = ozzr::plot_aspect(3)}
bp <- performance %>%
  dplyr::filter(experience!=params$reference) %>%
  ggplot2::ggplot() +
