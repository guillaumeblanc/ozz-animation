  ...
  */
  ...
  for (int e = 0; e < 2; e++) {
    ...
    for (int i = 0; i < i_skeleton.num_joints(); i++) {
      EXPECT_EQ(i_skeleton.joint_properties()[i].parent,
                o_skeleton->joint_properties()[i].parent);
      EXPECT_EQ(i_skeleton.joint_properties()[i].is_leaf,
                o_skeleton->joint_properties()[i].is_leaf);
    ...
    for (int i = 0; i < (i_skeleton.num_joints() + 3) / 4; i++) {
      EXPECT_TRUE(ozz::math::AreAllTrue(
        i_skeleton.bind_pose()[i].translation ==
        o_skeleton->bind_pose()[i].translation));
      EXPECT_TRUE(ozz::math::AreAllTrue(
        i_skeleton.bind_pose()[i].rotation ==
        o_skeleton->bind_pose()[i].rotation));
      EXPECT_TRUE(ozz::math::AreAllTrue(
        i_skeleton.bind_pose()[i].scale ==
        o_skeleton->bind_pose()[i].scale));
    }
  }
