...
#ifndef OZZ_DEMO_FRAMEWORK_IMGUI_H_
#define OZZ_DEMO_FRAMEWORK_IMGUI_H_
...
class ImGui {
 public:
  ...
  class Form {
   public:
    Form(ImGui* _im_gui,
         ...
         bool* _open)
      : im_gui_(_im_gui) {
        im_gui_->BeginContainer(_title, &_rect, _open);
    }
    ~Form() {
  ...
  class OpenClose {
   public:
    OpenClose(ImGui* _im_gui, const char* _title, bool* _open)
      : im_gui_(_im_gui) {
        im_gui_->BeginContainer(_title, NULL, _open);
    }
    ~OpenClose() {
      im_gui_->EndContainer();
    }
  ...
  virtual bool DoButton(const char* _label, bool _enabled = true) = 0;

  // Adds a slider to the current context and returns true if _value was
  // modified.
...
#endif  // OZZ_DEMO_FRAMEWORK_IMGUI_H_
