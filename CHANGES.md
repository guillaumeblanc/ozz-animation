...
* Library
  ...
  - [animation] Fixes memory read overrun in oz::animation::Skeleton while fixing up skeleton joint names.
...
* Build pipeline
  ...
  - Adds a clang-format target that re-formats all sdk sources. This target is not added to the BUILD_ALL target, it must be ran manually.
